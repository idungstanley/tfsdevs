...
import { ButtonProps } from '@/app/types/index.interface';
...
function Button({
  ...
  return (
    <button
      ...
      className={cl(
        disabled
          ? (`opacity-50 bg-gray-200 cursor-not-allowed text-gray-600 ${disableClasses} ${padding} ${height} ${width}` as string)
    ...
    >
      <div className="flex items-center gap-4">
        <div className="flex flex-row items-center gap-2">
          ...
          {icon && (iconPosition === 'left' || iconPosition === null) && <span className="">{icon}</span>}
